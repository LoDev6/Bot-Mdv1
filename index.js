...
async function startZaki() {
    ...
    const Zaki = ZakiConnect({
        ...
        browser: [' Windows 10 ','Safari','1.0.0'],
        auth: state,
        version
    })
    ...
    Zaki.sendMessage(callerId, { text: `Automatic block system!\nDon't call bot!\nPlease contact the owner to open ! : Https://wa.me/6289699348802`}, { quoted : pa7rick })
    await sleep(8000)
    ...
    Zaki.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./msg")(Zaki, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/3983c55ac7f3ebea225d3.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                if (anu.action == 'add') {
                    Zaki.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Hi 👋 Welcome To ${metadata.subject} @${num.split("@")[0]}` })
                } else if (anu.action == 'remove') {
                    Zaki.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split("@")[0]} Leave From ${metadata.subject}` })
                } else if (anu.action == 'promote') {
                    Zaki.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Promote From ${metadata.subject}` })
                } else if (anu.action == 'demote') {
                    Zaki.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Demote From ${metadata.subject}` })
              }
            }
        } catch (err) {
            console.log(err)
     ...
     * @returns
     */
     Zaki.sendButtonDocumen = (jid, text = '' , footer = '', but = [], quoted = '', options = {}) => {
    ...
    Zaki.sendMessage(jid, templateMessage, { quoted, ...options })
    }
     ...
     * @returns
     */
    Zaki.send5ButVid = async (jid , text = '' , footer = '', vid, but = [], options = {}) =>{
    let message = await prepareWAMessageMedia({ video: vid }, { upload: Zaki.waUploadToServer })
     ...
     * @returns
     */
    Zaki.send5ButGif = async (jid , text = '' , footer = '', gif, but = [], options = {}) =>{
    let message = await prepareWAMessageMedia({ video: gif, gifPlayback: true }, { upload: Zaki.waUploadToServer })
    var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
    templateMessage: {
    hydratedTemplate: {
    videoMessage: message.videoMessage,
